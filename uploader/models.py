...
def validate_file_size(value):
    if value.size > TEN_MEGABYTES_IN_BYTES:
        raise ValidationError(f'The maximum file size that can be uploaded is {TEN_MEGABYTES_IN_BYTES} bytes')
    else:
        return value


class Image(models.Model):
    picture = models.FileField(upload_to='images/', validators=[validate_file_size])
